use basic_blockchain_example;
use basic_blockchain_example::structures::block::Block;
use basic_blockchain_example::structures::blockchain::Blockchain;
use basic_blockchain_example::structures::transaction::{Transaction, TransactionData};
...
fn main() {
    ...
    let transaction_data = bc_attack_1.blocks[1].transactions.get_mut(0).unwrap();

    // change the amount value of the transaction INSIDE the chain
    match transaction_data.record {
       TransactionData::TransferTokens {to:_, ref mut amount} => {
    ...
    // Alice tokens
    let transaction_data= bc_attack_2.blocks[0].transactions.get_mut(1).unwrap();

    // change tokens
    match transaction_data.record {
        TransactionData::CreateTokens {receiver: _, ref mut amount} => {
